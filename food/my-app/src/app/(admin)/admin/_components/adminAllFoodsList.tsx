...
export const AllFoodList = ({ getCategory }: FoodsListProps) => {
  ...
  const getFoods = async () => {
    const response = await api.get(`/food?categoryId=${""}`);
          ...
          >
            ...
            <div className="grid grid-cols-4 gap-8">
              ...
              {filteredFoods.map((item) => (
                <div
                  key={item._id}
                  className="border rounded-xl shadow-md hover:shadow-xl transition-all duration-300 hover:scale-[1.02]"
                >
                  <AdminFoodCard
                    name={item.foodName}
                    price={item.price}
