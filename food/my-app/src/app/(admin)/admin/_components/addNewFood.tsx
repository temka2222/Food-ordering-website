...
export const AddNewFood=({categoryId}:FoodsListProps)=>{
     const { newFood, setNewFood } = useNewFood();
     const [category, setCategory] = useState<CategoryType[]>([]);
     const getCategory = async () => {
        const response = await axios.get(`http://localhost:3001/category?categoryId=${categoryId}`);
        ...
        <div className="flex h-[340px] flex-col bg-white rounded-2xl gap-5 p-4 border-dashed border-red-500 border-3 justify-center items-center">
              <Dialog>
                <DialogTrigger onClick={()=>{const newvalue = { ...newFood };
                          ...
                          setNewFood(newvalue);}} className="text-white w-9 h-9 flex justify-center items-center bg-red-500 rounded-full ml-4 ">
                  <PlusIcon size={18} />
                </DialogTrigger>

                <DialogContent className="йw-full h-[600px] ">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      <div className="flex flex-col w-[462px] h-[272px]  gap-10 justify-center pt-40">
                        <div className="flex flex-row gap-6 ">
                          ...
                          <div className="flex-1 flex flex-col gap-2 text-black">
                            ...
                            onChange={(e) => {
                        const newvalue = { ...newFood };
                        newvalue.price = e.target.value ? parseInt(e.target.value):0;
                        setNewFood(newvalue);
                      }}
                      ...
                      />
                        ...
                        <div className="w-full flex flex-col items-end">
                          <button onClick={()=>{addNewFood(), console.log(newFood)}} className="flex w-[93px] h-10 justify-center items-center bg-black text-white rounded-xl">
                            Add dish
                          </button>
                        </div>
                      </div>
