...
const AddToCart = (selectedFoodId: string, selectedQty: number) => {
  ...
  const duplicateFood=selectedFood?.filter((item)=>{return(item.foodId==selectedFoodId)})
  if(duplicateFood.length>0){
    const newCardFood=selectedFood.map((food)=>{
      return(food.foodId==selectedFoodId ? {...food,qty:food.qty+selectedQty}:food)
    })
    setSelectedFood(newCardFood)
  }
  else{
    setSelectedFood((prev)=>[...prev,{foodId: selectedFoodId, qty: selectedQty}])
    ...
    <div className="flex-1 h-full flex flex-col items-end">
      <div className="flex h-full w-full flex-col justify-between items-start">
        <div className="flex flex-col gap-3">
          <DialogTitle className="text-red-400 font-bold text-2xl">{foodName}</DialogTitle>
