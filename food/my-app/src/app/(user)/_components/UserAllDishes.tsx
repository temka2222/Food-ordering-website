...
import { FoodCard } from "@/app/_components/productCard";
  ...
  <div className="flex flex-col gap-[54px] pt-22 pr-22 pl-22">
    {category.map((element) => {
      ...
      return (
        <div
          key={element._id}
          className="w-full bg-[#404040] p-8 rounded-3xl shadow-lg border-2  border-gray-300  hover:shadow-xl transition-all duration-300"
        >
          ...
          <div className="grid grid-cols-4 gap-8">
            {filteredFoods.map((item) => (
              <div
                key={item._id}
                className=" rounded-xl shadow-md hover:shadow-lg transition-all duration-300 hover:scale-[1.02] hover:border-[#ff6347]"
              >
                <FoodCard
